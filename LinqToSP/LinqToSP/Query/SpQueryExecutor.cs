...
namespace SP.Client.Linq.Query
{
    ...
    where TEntity : class, IListItemEntity
  {
    ...
    protected virtual IEnumerable<object> GetEntities(Type type)
    {
      ...
      var entities = Enumerable.Empty<ListItemEntity>();
      ...
      do
      {
        if (SpQueryArgs.BatchSize > 0)
        {
          if (rowLimit > 0)
          {
            SpView.Limit = Math.Min(rowLimit - itemCount, SpQueryArgs.BatchSize);
          }
          else
          {
            SpView.Limit = SpQueryArgs.BatchSize;
          }
          if (SpView.Limit == 0)
          {
            break;
        ...
        var items = GetItems(SpQueryArgs, SpView, position);
        if (items != null)
        {
    ...
    {
      if (!typeof(IListItemEntity).IsAssignableFrom(type) && !type.IsSubclassOf(typeof(IListItemEntity)))
      {
        throw new Exception($"Entity must be assignable from {typeof(IListItemEntity)}");
      }
    }
    ...
    protected static ListItemCollection GetItems(SpQueryArgs<TContext> args, SpView spView, ListItemCollectionPosition position)
    {
