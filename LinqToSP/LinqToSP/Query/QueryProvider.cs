...
{
  public class QueryProvider<TEntity> : QueryProviderBase
  where TEntity : class, IListItemEntity
  {
    ...
    private IQueryable<TEntity> CreateQueryInternal(Expression expression)
    {
      if (expression is IncludeExpression)
      {
        if (Executor is SpQueryExecutor<TEntity, ISpEntryDataContext>)
        {
          (Executor as SpQueryExecutor<TEntity, ISpEntryDataContext>).IncludeExpressions.Add(expression as IncludeExpression);
        }
      ...
      if (expression is GroupByExpression)
      {
        if (Executor is SpQueryExecutor<TEntity, ISpEntryDataContext>)
        {
          (Executor as SpQueryExecutor<TEntity, ISpEntryDataContext>).GroupByExpressions.Add(expression as GroupByExpression);
        }
...
}), new object[]
{
    ...
    {
      return (IQueryable<T>)CreateQueryInternal(expression);
    }
  }
}
