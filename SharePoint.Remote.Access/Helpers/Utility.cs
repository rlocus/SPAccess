    ...
    public static class Utility
    {
        ...
        public static string CombineUrls(Uri baseUri, string relativeUrl)
        {
            ...
            var siteCollectionUrl =
                $"{baseUri.Scheme}://{baseUri.DnsSafeHost}{(baseUri.LocalPath.Equals("/") ? string.Empty : baseUri.LocalPath)}/";

            var webUrl = relativeUrl.TrimEnd('/').Replace(new Uri(siteCollectionUrl).LocalPath.TrimEnd('/'), "");
            var url = $"{siteCollectionUrl.TrimEnd('/')}/{webUrl.TrimStart('/')}";
            return url;
        }
